...
const emit = defineEmits(['update:llamaPort', 'update:webuiPort', 'update:allowedSubnets', 'update:installDir'])
</script>
  ...
  <div class="space-y-4 pt-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="space-y-1.5">
        <label class="text-xs font-medium text-zinc-300">Puerto llama.cpp (API)</label>
        <input
          type="number"
          :value="llamaPort"
          @input="$emit('update:llamaPort', $event.target.value)"
          placeholder="3000"
        ...
        >
        <span v-if="errors?.llamaPort" class="text-[10px] text-rose-400 block">{{ errors.llamaPort }}</span>
        <span v-else class="text-[10px] text-zinc-500 block">Default: 3000</span>
      </div>
      ...
      <div class="space-y-1.5">
        ...
        <input
          type="number"
          :value="webuiPort"
          @input="$emit('update:webuiPort', $event.target.value)"
          placeholder="8080"
          class="w-full bg-zinc-950/60 border border-zinc-900 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-zinc-800 placeholder-zinc-700"
        >
      ...
      <div class="space-y-1.5 sm:col-span-2">
        <label class="text-xs font-medium text-zinc-300">Subredes Permitidas en UFW/Firewall</label>
        <input
          type="text"
          :value="allowedSubnets"
          @input="$emit('update:allowedSubnets', $event.target.value)"
      ...
      <div class="space-y-1.5 sm:col-span-2">
        ...
        <input
          type="text"
          :value="installDir"
          @input="$emit('update:installDir', $event.target.value)"
          placeholder="$HOME/ai-stack"
          class="w-full bg-zinc-950/60 border border-zinc-900 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-zinc-800 font-mono text-xs placeholder-zinc-700"
        >
        <span class="text-[10px] text-zinc-500 block">Default: Carpeta del usuario ($HOME/ai-stack)</span>
      </div>
    </div>
  </div>
</template>
